...
export const Footer = () => {
  ...
  useEffect(() => {
    ...
    const modals = actionValues.map((item, index) => (checkboxes[index] ? item : undefined)).filter((i) => i) as (
      | 'before'
      | 'success'
      | 'error'
    )[]
      ...
      <div>
        <label>
          ...
          <input
            style={{ width: '200px' }}
            value={walletsSelect}
            onChange={(e) => setWalletsSelect(e.target.value)}
            onBlur={onWalletsInputBlur}
          />
        </label>
      </div>
