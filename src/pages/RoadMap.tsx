...
import { Button, Card, Col, Grid, Loading, Row, Spacer, Table, Text } from '@nextui-org/react'
...
export const RoadMap = () => {
  ...
  return (
    <Grid.Container alignItems="center" justify="center" css={{ minHeight: 'calc(100vh - 57px)' }}>
      <Grid>
        <Grid.Container id="roadmap" gap={2} justify="center" alignItems="center" css={{ mt: 32 }}>
          <Grid>
            <Button
              icon={<ARR02 style={{ fill: 'var(--nextui-colors-link)', fontSize: 32 }} />}
              flat
              css={{ minWidth: 24 }}
          ...
          <Grid>
            <Button
              icon={<ARR01 style={{ fill: 'var(--nextui-colors-link)', fontSize: 32 }} />}
              flat
              css={{ minWidth: 24 }}
            ...
            <div className="scene">
              <div className="carousel keen-slider" ref={sliderRef}>
                {roadMap.map(({ ref, step, list }, i) => (
                  <div key={`${step}-${i}`} className={cn('carousel__cell', 'number-slide', { active: active === i })}>
                    <Card variant="bordered">
                      <Card.Header css={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Text size="$2xl" weight="bold" css={{ textGradient: '45deg, $blue600 0%, $green600 100%' }}>
                          {step}
                        </Text>
                        <Row css={{ flexDirection: 'column', w: 'auto', color: '$accents3' }}>
                          <Col>
                            <ARR03 style={{ fill: 'var(--nextui-colors-link)', fontSize: 32 }} />
                          </Col>
                          <Col>
                            <ARR04 style={{ fill: 'var(--nextui-colors-link)', fontSize: 32 }} />
                          </Col>
                        </Row>
                      </Card.Header>
                      <Card.Body ref={ref} className="roadmap-body">
                        <Table aria-label="RoadMap" className="table-hide-header" {...tableProps}>
                          <Table.Header>
                          ...
                          <Table.Body>
                            {list.map((item, y) => {
                              let icon = <ARR12 style={{ fill: 'var(--nextui-colors-link)', fontSize: 32 }} />

                              switch (item.type) {
                                case 'vacation':
                                  icon = <Loading size="xs" color="currentColor" />
                                  break
                                case 'paused':
                                  icon = <Loading type="points-opacity" size="xs" color="currentColor" />
                                  break
                              }
                                ...
                                <Table.Row key={`${step}-${i}-${y}`}>
                                  <Table.Cell>
                                    <Row align="center" justify="flex-start">
                                      <Col css={{ w: 'auto', display: 'flex' }}>
                                        <ARR10 style={{ fill: '$accents5', fontSize: 32 }} />
                                      </Col>
                                      <Spacer x={0.5} />
                                      <Col style={{ maxWidth: 242, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                        {item.title}
                                      </Col>
                                    </Row>
                                  </Table.Cell>
